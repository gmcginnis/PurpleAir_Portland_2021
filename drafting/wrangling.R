...
## ([0-9])\w+\.([0-9])\w+\s\-([0-9])\w+\.([0-9])\w+ #some RegEx that nabs coordinates

#gets list of files which are outside sensors only
#my_files <- list.files(path = "purpleair_drafting/blob/", pattern = ".+\\(*(outside*)\\w+\\)")
my_file_paths <- list.files(path = "drafting/blob", pattern = ".+\\(*(outside*)\\w+\\)", full.names = TRUE)

...
get_slope <- function(input_pa_var){
  se_laf %>%
    group_by(range) %>%
    summarize(
    slope = get_regression_table(lm(deq~{{input_pa_var}}))$estimate[2],
    error = get_regression_table(lm(deq~{{input_pa_var}}))$std_error[2]
  ) %>% 
  unite("ouput_equation", c(slope, error), sep = " \u00B1 ")
}
...
se_laf %>% 
  group_by(range) %>% 
  summarize(
    slope = get_regression_table(lm(deq~uncorr))$estimate[2],
    error = get_regression_table(lm(deq~uncorr))$std_error[2]
  ) %>% 
  unite("slope_error", c(slope, error), sep = " \u00B1 ")

# se_laf %>% 
#   group_by(range) %>% 
#   summarize_at(
#         c("uncorr", "epa_2020", "epa_2021"),
#         slope = get_regression_table(lm(deq~.x))$estimate[2]
#         # list(
#         #   slope = get_regression_table(lm(deq~.x))$estimate[2],
#         #   error = get_regression_table(lm(deq~.x))$std_error[2]
#         # )
#   )
...
se_laf %>% 
  ...
  facet_grid(
    pa_name~range,
    #scales = "free",
    labeller = labeller(pa_name = facet_corr, range = facet_range)
  ) +
  geom_point(shape = 3, alpha=0.5) +
  theme_igray() +
  stat_smooth(method = lm) +
  geom_text(data = df_slopes, aes(x = 600, y = 50, label = slope), color = "blue", size = 3) +
  labs(
    title = lab_PM,
    subtitle = "Scatterplot with slope (blue) and underlayed 1:1 ratio line",
    caption = "Data from SE Laf. STAR lab PA sensor (CF1 outputs), and co-located DEQ sensor.
    Columns represent different time range subsets. Rows represent EPA correction factors.",
    x = "PA",
    y = "DEQ"
  )

#ggsave(lafayette_pa_deq_EPA, file = "/figures/lafayette_pa_deq_EPA.png", width = 10)
```

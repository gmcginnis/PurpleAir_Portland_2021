...
data_bc %>% 
  ...
  rbind((data_pm25 %>% select(datetime, pm25_atm) %>% pivot_longer(where(is.numeric)))) %>% 
  filter(datetime <= "2021-07-11 11:00:00 PDT") %>% 
  mutate(hour = hour(datetime)) %>% 
  group_by(name, hour) %>% 
  mutate(max = case_when(value == max(value, na.rm = TRUE) ~ value)) %>% 
  ggplot(aes(datetime, value, color = name,
             label = paste0(round(max, 2), " (", format(datetime, "%H:%M:%S"), ")"))
  ) +
  geom_path(alpha = 0.3) +
  ...
  geom_text_repel(
    data = . %>%
      select(!max) %>% 
      filter(
        str_detect(name, "pm25"),
        datetime >= "2021-07-10 22:30:00 PDT",
        datetime <= "2021-07-10 22:59:00 PDT"
      ) %>% 
      group_by(hour) %>% 
      mutate(max = case_when(value == max(value, na.rm = TRUE) ~ value)) %>% 
      drop_na(max),
  ...
  geom_text_repel(
    ...
    data = . %>% drop_na(max) %>% filter(str_detect(name, "pm25")),
    size = 3,
    color = "blue",
    min.segment.length = 0,
    nudge_y = 2
  ...
  geom_text_repel(
    data = . %>% drop_na(max) %>% filter(name == "BC"),
    size = 3,
    color = "red",
    min.segment.length = 0
  ) +

...
```{r getting_pas}
get_area_pas <- function(state_code = NULL, w, e, s, n){
...
map_q <- function(pm_data, value, location_data = data_meta,
  ...
  if(is.null(exclude) == FALSE){
    ...
    #print("Exclusions for grouping variable 1 applied.")
    print(paste("Excluding", exclude, "from", deparse(substitute(grouping_var))))
  }
  if(is.null(exclude_2) == FALSE){
    to_exclude_2 <- paste(exclude_2, collapse = "|")
    data_temp <- data_temp %>% 
      filter(!str_detect({{grouping_var_2}}, to_exclude_2))
    #print("Exclusions for grouping variable 2 applied.")
    #print(paste("Excluding the following from", grouping_var_2, "applied:", exclude_2))
    print(paste("Excluding", exclude_2, "from", deparse(substitute(grouping_var_2))))
  }
  ...
  if(outside == FALSE){
    ...
    print("Outdoor sensor data has been filtered out")
    shapes_inout <- c("inside" = 23)
  }
  if(inside == FALSE){
    data_temp <- data_temp %>% 
      filter(DEVICE_LOCATIONTYPE != "inside")
    print("Indoor sensor data has been filtered out")
...
map_q(data_daily, value = epa_2021, grouping_var = date_tag, outside = FALSE)

map_q(data_daily, value = lrapa, grouping_var = date_tag, exclude = "Fire")
map_q(data_hourly, value = lrapa, grouping_var = date_tag, grouping_var_2 = hour_tag)

...
# REQUIRED: Start and end dates of interest. Please use the following format: "YYYY-MM-DD", with leading zeros where appropriate
input_startdate <- "2021-06-01"
input_enddate <- "2021-06-30"
...
for (id in ids[1:id_count]) {
  ...
  # If there is more than a single pair of dates in the date sequence
...
raw_data <- full_join(data_a, data_b, # Data sets to join
                      ...
                      suffix = c("_A", "_B")) %>% # Adding suffixes for distinct columns
...
data_pm25 <- raw_data %>% 
  # Selecting only variables of interest, to save on storage
  #select(site_id, datetime, temperature, humidity, pm25_cf1_A, pm25_cf1_B, pm25_atm_A, pm25_atm_B) %>% 
...
# data_daily <- apply_corrections(data_pm25, daily = TRUE, hourly = FALSE)
# data_hourly <- apply_corrections(data_pm25, daily = TRUE, hourly = TRUE)
```


```{r corr_epa_factor_old, eval = FALSE}
apply_epa_corrections_old <- function(dataset, groupings){
  dataset %>% 
    group_by_at({{groupings}}) %>% 
    mutate(
      # Difference between A&B sensors
...
```{r lrapa_old, eval = FALSE}
apply_lrapa_correction_old <- function(dataset, groupings){
  dataset %>% 
    group_by_at({{groupings}}) %>% 
    summarize_if(is.numeric, mean, na.rm = TRUE) %>% 
    rowwise() %>% 
...
#   data_hourly <- apply_date_tags(data_hourly)
#   data_daily <- apply_date_tags(data_daily)
...
data_pm25 %>% 
  applying_functions(group_day = FALSE, group_hour = TRUE)

data_pm25 %>% 
  applying_functions(group_day = TRUE, group_hour = FALSE)
...
```{r}
data_daily <- data_pm25 %>% 
  apply_corrections(daily = TRUE, hourly = FALSE) %>% 
  apply_date_tags()

data_hourly <- data_pm25 %>% 
  apply_corrections(daily = TRUE, hourly = TRUE) %>% 
  apply_hour_tags() %>% 
  apply_date_tags()

data_diurnal <- data_pm25 %>% 
  apply_corrections(daily = FALSE, hourly = TRUE) %>% 
  apply_hour_tags()
```
...
```{r saving_fire, eval=FALSE}
# Saving data sets for fire for ease of loading/practicing viz
# save(data_meta, file = "data/summer/sep2020_meta.rds")
# save(data_pm25, file = "data/summer/sep2020_pm25.RData")
# save(data_hourly, file = "data/summer/sep2020_pm25_hourly.RData")
# save(data_daily, file = "data/summer/sep2020_pm25_daily.RData")

save(data_pm25, data_meta, data_daily, data_hourly, include_outside, include_inside, file = "data/summer/sep2020.RData")

#save.image(file = "data/summer/sep2020.RData")
```

```{r loading_fire, eval = FALSE}
load("data/summer/sep2020.RData")
```


```{r map_tests, eval = FALSE}
map_q(data_hourly, value = epa_2021, grouping_var = hour_tag)
map_q(data_hourly, value=epa_2021)
map_q(data_hourly, value = epa_2021, grouping_var = hour_tag, grouping_var_2 = date_tag)
map_q(data_hourly, value = pm25_cf1, grouping_var = hour_tag, grouping_var_2 = date_tag)
map_q(data_hourly, value = pm25_atm, grouping_var = hour_tag, grouping_var_2 = date_tag)

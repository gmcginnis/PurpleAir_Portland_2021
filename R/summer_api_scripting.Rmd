...
map_q(data_daily, variable_of_interest = pm25_atm, grouping_vars = "date_tag", cap_value = 12)
...
```{r}
data_daily %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  map_q(., pm25_epa_2021)

data_daily %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  map_q(., pm25_epa_2021, "date_tag")

data_hourly %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  heatmap_single(pm25_epa_2021, "STAR", dataset = .)

data_hourly %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  # filter_df(., include = c("STAR", "SE", "NCA")) %>% 
  heatmap_cross(., pm25_epa_2021, drop_incomplete = TRUE, cap_value = 70, cap_color = "green")
...
data_hourly %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  heatmap_single(pm25_epa_2021, "Harrison", dataset = .)

data_hourly %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  ts_line(., pm25_epa_2021, label = c("STAR", "Woodlands"), add_points = TRUE)

data_hourly %>% 
  rename(pm25_epa_2021 = epa_2021) %>% 
  ts_line(., pm25_epa_2021, label = c("STAR"), include_avg = FALSE)
```
...
rez <- add_cap_new(data_daily, "pm25_atm", 10, "red", "flag")
...
ts_capped <- function(dataset, variable_of_interest,
  ...
  extrema <- dataset %>%
    left_join({{location_data}}) %>%
    filter(str_detect(label, input_labels)) %>%
    select(site_id, timestamp, {{variable_of_interest}}) %>%
    group_by(site_id) %>%
    mutate(
      date = date(timestamp),
    ...
    mutate_at(vars(max, min), ~replace(., duplicated(.), NA)) %>%
    ungroup() %>% 
    select(site_id, timestamp, {{variable_of_interest}}, max, min)
  ...
  if(include_avg == TRUE){
    ...
    extrema_averages <- dataset %>%
      ...
      filter_at(vars(max, min), any_vars(!is.na(.))) %>% 
      group_by(site_id, date) %>% 
      mutate_at(vars(max, min), ~replace(., duplicated(.), NA)) %>%
      ungroup() %>% 
      select(site_id, timestamp, max, min) %>% 
      filter_at(vars(max, min), any_vars(!is.na(.)))

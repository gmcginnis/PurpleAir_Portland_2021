...
apply_functions <- function(dataset,
                            by_day = TRUE, by_hour = FALSE,
                            tag_dates = run_date_grouping, tag_hours = run_hour_grouping){
...
if (run_date_grouping == TRUE){
  data_pm25 <- data_pm25 %>% apply_date_tags()
}
if (run_hour_grouping == TRUE) {
  data_pm25 <- data_pm25 %>% apply_hour_tags()
}
```
...
```{r ts_variation_testing, eval = FALSE}
ts_variation(data_hourly, "pm25_atm", include = "Lighthouse", group = "date_tag")
ts_variation(data_daily, c("pm25_atm", "temperature"))

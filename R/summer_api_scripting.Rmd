...
heatmap_single(data_hourly, "Sunset", epa_2021)
```
...
heatmap_cross <- function(dataset, variable_of_interest, drop_incomplete = FALSE,
  ...
  if (str_detect(variable_of_interest_qt, "pm") == TRUE) {
    ...
    if (str_detect(variable_of_interest_qt, "(1.0)|(01)|(1$)|(1\\D)") == TRUE) {
      pm_val <- 1.0
      # lab_fill <- expression(paste("PM"[1.0]*" (", mu, "g/m"^3*"):"))
      ...
      pm_val <- 10
      # lab_fill <- expression(paste("PM"[10]*" (", mu, "g/m"^3*"):"))
      print("PM 10 detected")
    } else {
      pm_val <- NULL
      # lab_fill <- expression(paste("PM (", mu, "g/m"^3*"):"))
  ...
  val_min <- dataset %>% select({{variable_of_interest}}) %>% min(na.rm = TRUE)
  val_max <- dataset %>% select({{variable_of_interest}}) %>% max(na.rm = TRUE)
...
heatmap_cross(data_hourly, epa_2021)
heatmap_cross(data_daily, temperature)
heatmap_cross(data_diurnal, epa_2021)
```


```{r viz_function_ts}
ts_line <- function(dataset, variable_of_interest, include_avg = TRUE,
                    arrangement = "NULL",
                    label = c(""), location_data = data_meta, digits = 2){
  
  # Dropping NA values from the variable of interest
  dataset <- dataset %>% 
    rename("var" = {{variable_of_interest}}) %>% 
    drop_na(var)
  ...
  rounding <- function(var, input_digits = digits) {
    as.character(
      sprintf(
        label_digits,
        round({{var}}, digits = input_digits)
      )
    )
  }
  
  if (arrangement == "column" | arrangement == "rows") {
    facet <- facet_grid(label~.)
    print("Charts will be arranged in a single column (multiple rows).")
  # } else if (arrangement == "row" | arrangement == "columns") {
  #   facet <- facet_grid(.~label)
  #   print("Charts will be arranged in a single row (multiple columns).")
  } else {
  ...
  dataset %>% 
    ...
    drop_na(var) %>%
    mutate(
      max = case_when(!is.na(max) ~ rounding(max)),
      min = case_when(!is.na(min) ~ rounding(min))
    ) %>% 

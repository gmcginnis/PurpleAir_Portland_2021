...
```{r, eval = FALSE}
starts <- as.Date(dategroup_starts)
ends <- as.Date(dategroup_ends)
tags <- factor(tags)
date_stamp <- stamp_date("17 Jan 1999")

data.frame(starts, ends, tags) %>% 
    arrange(starts) %>% 
    mutate(
      start_stamp = date_stamp(starts),
      end_stamp = date_stamp(ends),
      #date_tag = fct_inorder(tags),
      date_tag = fct_inorder(paste0(tags, " (", start_stamp, " to ", end_stamp, ")"))
      #date_tag = fct_reorder(date_tag, tags)
    ) %>% 
    #) %>% 
    pivot_longer(cols = c(starts, ends), values_to = "date") %>% 
    group_by(date_tag) %>% 

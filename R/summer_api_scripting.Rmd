...
input_startdate <- "2021-07-09"
input_enddate <- "2021-07-14"
...
# Example format (capitalization matters!): input_labels <- c("se", "SE", "Se", "STAR", "PSU")
input_labels <- "Indigo"
...
heatmap_single <- function(dataset, site_of_interest, variable_of_interest,
  ...
  min_date <- format(min(date(temp_df$date_hour)), "%d %b %Y")
  max_date <- format(max(date(temp_df$date_hour)), "%d %b %Y")
  
  lab_title <- expression("Heatmap of PM"[2.5]*" values across time")
  lab_subtitle <- paste0("PurpleAir data of monitor \"", unique(temp_df$label), "\"\nfrom ",
  ...
  if (str_detect(deparse(substitute(variable_of_interest)), "temp") == TRUE) {
    ...
    if (str_detect(deparse(substitute(variable_of_interest)), "temperature_c") == TRUE) {
      lab_fill <- "Internal temperature (\u00B0C):"
  ...
  if (str_detect(deparse(substitute(variable_of_interest)), "humid") == TRUE) {
    ...
    fill_colors <- scale_fill_viridis(option = "mako", direction = -1, limits = c(0, 100), end = 0.95, begin = 0.15)
...
heatmap_cross <- function(dataset, variable_of_interest, drop_incomplete = FALSE,
  ...
  # Adding quotation marks; removing extra pairs as needed
  max_color <- deparse(substitute(max_color)) %>% str_replace_all("\\\"", "")
  ...
  lab_fill <- expression(paste("PM"[2.5]*" (", mu, "g/m"^3*"):"))
  lab_subtitle <- paste0("Variable plotted: ",
                         ...
                         " to ",
                         round(val_max, digits = digits), " units.")
  lab_caption <- paste0("Data from PurpleAir, from ",
                        start_date, " to ", end_date, ".",
                        "\nMonitors are arranged north to south in their respective groups.")
  
  cap_color <- guides(fill = guide_colorbar(order = 1, barwidth = 10),
                      color = "none")
  ...
  if (is.na(filter_max) == FALSE) {
    ...
    dataset <- dataset %>% 
      mutate_at(vars({{variable_of_interest}}), ~replace(., which(.>{{filter_max}}), NA))
    
    # Updated lab caption to include the filter
    lab_subtitle <- paste0(lab_subtitle, "\nColor scale manually capped at ", filter_max, "; all higher values colored ", max_color, ".")
    
    # Feedback
    print(paste("Values greater than",
                {{filter_max}},
                "in",
                deparse(substitute(variable_of_interest)),
                "will be colored",
    ...
    cap_color <- guides(fill = guide_colorbar(order = 1, barwidth = 10),
                        color = guide_legend(title = paste0(filter_max, "+"),
                                             order = 2,
                                             title.position = "bottom",
                                             title.theme = element_text(size = 10),
                                             override.aes=list(color = max_color, fill = max_color)))
  }
  ...
  fill_colors <- scale_fill_viridis(option = "plasma", limits = c(0, NA), na.value = max_color)
  
  # Adjusting color scale and labels if the variable of interest is internal temperature
  if (str_detect(deparse(substitute(variable_of_interest)), "temperature") == TRUE) {
  ...
  if (str_detect(deparse(substitute(variable_of_interest)), "humid") == TRUE) {
    #lab_title <- paste("Heatmap of humidity")
  ...
  if ("datetime" %in% colnames(dataset) == TRUE) {
    ...
    lab_title_sub <- "\nacross time, averaged hourly by day"
    ...
    lab_title_sub <- "\nacross time, averaged by hour of day"
...
#heatmap_cross(data_pm25, variable_of_interest=pm25_atm, filter_max = 50, drop_incomplete = TRUE)
heatmap_cross(data_hourly, variable_of_interest=epa_2021, start_date = min(data_daily$date))
heatmap_cross(data_daily, variable_of_interest=epa_2021, start_date = min(data_daily$date))
heatmap_cross(data_diurnal, variable_of_interest=epa_2021, start_date = min(data_daily$date))
```

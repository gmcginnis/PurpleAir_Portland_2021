...
# Run date grouping?
run_dategrouping <- FALSE
# Date grouping categories
if(run_dategrouping == TRUE){
  dategroup_tags <- c("Before", "Later")
  dategroup_starts <- c("2021-03-13", "2021-03-15")
  dategroup_ends <- c("2021-03-14", "2021-03-16")
} else {
...
data_pm25 <- raw_data %>% 
  ...
  # Adjusting to be local time zone
  adjust_timezone() %>% 
  ...
  select(site_id, datetime_utc, datetime, date, hour, everything())
...
```{r apply_dates_set}
apply_dates <- function(dataset, starts = dategroup_starts, ends = dategroup_ends, tags = dategroup_tags){
...
if(run_dategrouping == TRUE){
  data_pm25_epa <- apply_dates(data_pm25_epa)
  data_pm25_lrapa <- apply_dates(data_pm25_lrapa)
...
map_q <- function(pm_data, location_data = data_meta, correction = epa_2021,
...
map_q(data_pm25_epa, point_size = 6)
map_q(data_pm25_lrapa, point_size = 5, correction = lrapa)
```

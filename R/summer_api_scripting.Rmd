...
```{r label_tests, eval = FALSE}
#labels_interest <- c("Benson", "PSU STAR LAB SEL")
labels_interest <- c("Benson", "PSU", "STAR")
labels_of_interst <- paste0("(",paste(labels_interest, collapse = ")|("), ")")
remove(labels_interest)

pas_load(archival = TRUE, datestamp = str_replace_all(as.character(enddate), "-", "")) %>% 
  pas_filter(str_detect(label, labels_of_interst))
...
map_q <- function(pm_data, value, location_data = data_meta,
  ...
  labels_of_interest <- paste0("(", paste(label, collapse = ")|("), ")")
  ...
  data_temp <- pm_data %>% 
    ...
    filter(str_detect(label, labels_of_interest))

...
```{r viz_heatmap_single}
heatmap_single <- function(dataset, site_of_interest, variable_of_interest, location_data = data_meta){
  ...
  if(length(unique(temp_df$label)) == 0){stop("ERROR: No monitors selected. Please verify a distinct label was provided.")}
  ...
  #lab_subtitle <- paste0("Data from PurpleAir. Monitor selected: \"", site_of_interest,
  ...
  temp_df %>% 
    ggplot(aes(x = date,
               ...
               label = as.character(sprintf("%.2f", round({{variable_of_interest}}, digits=2)))
    )) +
    geom_tile() +
...
```{r viz_heatmap_mass}
heatmap_cross <- function(dataset, variable_of_interest, location_data = data_meta, digits = 2, filter_max = NA){
  temp_loc <- location_data %>% 
  ...
  lab_pm <- expression(paste("Average PM"[2.5]*" (", mu, "g/m"^3*"):"))
  lab_caption <- paste("Maximum value reported:",
                       round(
                         max(
                           dataset[,deparse(substitute(variable_of_interest))],
                           na.rm=TRUE
                         ),
                         digits = 2
                       ))
  ...
  if(is.na(filter_max) == FALSE){
    ...
    lab_caption <- paste("Values manually capped at:", filter_max)
    
    print(paste(
      "Filtered out values greater than",
      {{filter_max}},
      "from",
...
heatmap_cross(data_daily, variable_of_interest=epa_2021, filter_max = 50)
heatmap_cross(data_diurnal, variable_of_interest=epa_2021, filter_max = 105)
heatmap_cross(data_diurnal, variable_of_interest=epa_2021)
```
...
data_hourly %>% 
  ...
  #y = str_wrap(label, width = 30)
  ggplot(aes(x = date_hour, y = label, fill = epa_2021, label = as.character(sprintf("%.2f", round(epa_2021, digits=2))))) +
  facet_grid(DEVICE_LOCATIONTYPE~., scales = "free_y", space = "free_y") +
  geom_tile() +
  # plasma or mako w direction -1
  #scale_fill_viridis(option = "mako", direction = -1, limits = c(0, NA)) +
  scale_fill_viridis(option = "plasma", limits = c(0, NA)) +
  scale_y_discrete(limits = rev) +
  #scale_y_discrete(guide=guide_axis(n.dodge=2)) +
  # scale_y_discrete(breaks = data_meta$label[seq(1, length(data_meta$label), by=2)])
  scale_x_datetime(
    breaks = "1 day",
    date_labels = "%d %b %Y",

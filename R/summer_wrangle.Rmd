...
data_pa <- data_pa_raw %>% 
  ...
  apply_qc() %>% 
  adjust_timezone(location_data = data_loc) %>% 
  select(site_id, datetime, date, date_hour, everything())
```

```{r auto_apply}
data_pa_hourly <- applying_functions(data_pa, group_day = TRUE, group_hour = TRUE)
data_pa_daily <- applying_functions(data_pa, group_day = TRUE, group_hour = FALSE)
data_pa_diurnal <- applying_functions(data_pa, group_day = FALSE, group_hour = TRUE)
```
...
data_pa_hourly %>%
  map_q(value = pm25_atm, location_data = data_loc, grouping_var = hour_tag)

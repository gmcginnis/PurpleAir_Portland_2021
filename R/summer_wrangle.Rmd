...
```{r corrections}
apply_correction_factors<- function(data, grouping_vars){
  epa <- data %>% 
    apply_epa_corrections(groupings = grouping_vars)
  
  lrapa <- data %>% 
    apply_lrapa_correction(groupings = grouping_vars)
...
data_pa_hourly <- data_pa %>% 
  apply_correction_factors(grouping_vars = vars(label, date, date_hour)) %>% 
  apply_date_grouping() %>% 
  apply_hour_grouping()

data_pa_daily <- data_pa %>% 
  apply_correction_factors(grouping_vars = vars(label, date)) %>% 
  apply_date_grouping()

...
calendarPlot(
  data_daily, pollutant = "pm25_epa_2021", year = 2021,
  annotate = "value",
  cols = "Purples"
)
...
data_hourly %>% 
  ...
  timeVariation(pollutant = c("pm25_lrapa", "pm25_atm", "pm25_epa_2021"))


data_hourly %>% 
  rename(date = date_hour) %>% 
  timeVariation(pollutant = c("pm25_epa_2020", "pm25_epa_2021", "epa_atm"))
...
data_hourly %>% 
  ...
  # apply_date_tags() %>% 
  rename(date = date_hour) %>% 
  timeVariation(pollutant = "pm25_epa_2021", group = "date_tag", cols = "Dark2")
...
data_hourly %>% 
  ...
  select(site_id, date_hour, date_tag, hour_tag, pm25_epa_2021) %>% 
...
data_hourly %>% 
  ...
  apply_date_tags() %>% 
  ggplot(aes(x = hour, y = pm25_epa_2021, group = date, color = tag)) +
...
data_hourly %>% 
  ...
  ggplot(aes(x = hour, y = pm25_epa_2021, group = date, color = date_tag)) +
  coord_polar() +
  geom_line()
...
data_hourly %>% 
  filter(site_id == "6c18a7181a09037c_47107") %>% 
  column_dt(c("date", "hour")) %>% 
  ggplot(aes(x = hour, y = pm25_epa_2021, group = date, color = date)) +
  geom_line() +
  geom_point()
...
data_hourly %>% 
  ...
  drop_na() %>% 
  mutate(per_error = ((pm25_epa_2021-pm25_deq)/pm25_deq)*100) %>% 
  distinct() %>% 
...
data_hourly %>% 
  drop_na() %>% 
  left_join(data_meta) %>% 
  ggplot(aes(x = date_hour, y = pm25_epa_2021, group = site_id)) +
  facet_wrap(~label) +
...
data_hourly %>% 
  ...
  # facet_wrap(~label) +
  geom_line(data = (data_hourly %>% drop_na()), color = "gray") +
  geom_line(data = (data_deq %>% drop_na(pm25_deq) %>% rename(pm25_epa_2021 = pm25_deq)), color = "green") +
  geom_line(color = "red") +
  theme_minimal() +
...
hourly_min <- data_hourly %>% 
  drop_na(pm25_epa_2021) %>% 
  left_join(data_meta) %>% 
  group_by(site_id) %>% 
  filter(pm25_epa_2021==min(pm25_epa_2021)) %>%
  distinct() %>% 
  select(site_id, label, date_hour, pm25_epa_2021) %>% 
  mutate(epa_2021_text = as.character(sprintf(paste0("%.", "2", "f"), round(pm25_epa_2021, digits = 2))))

hourly_max <- data_hourly %>% 
  drop_na(pm25_epa_2021) %>% 
  left_join(data_meta) %>% 
  group_by(label) %>% 
  filter(pm25_epa_2021==max(pm25_epa_2021)) %>%
  distinct() %>% 
  select(site_id, label, date_hour, pm25_epa_2021) %>% 
  mutate(epa_2021_text = as.character(sprintf(paste0("%.", "2", "f"), round(pm25_epa_2021, digits = 2))))
...
data_hourly %>% 
  ...
  left_join(data_meta) %>% 
  filter(str_detect(label, "SE \\D")) %>%
  ggplot(aes(x = date_hour, y = pm25_epa_2021)) +
  facet_grid(label~.) +
  geom_line(data = (data_hourly %>% drop_na()), color = "gray", aes(group = site_id)) +
  geom_line(data = (data_hourly %>% ungroup() %>% group_by(date_hour) %>% summarize(pm25_epa_2021=mean(pm25_epa_2021,na.rm=TRUE))), color = "green") +
...
extrema <- data_hourly %>% 
  ...
  group_by(site_id) %>% 
  mutate(
    max = case_when(pm25_epa_2021 == max(pm25_epa_2021, na.rm=TRUE) ~ pm25_epa_2021),
    min = case_when(pm25_epa_2021 == min(pm25_epa_2021, na.rm=TRUE) ~ pm25_epa_2021),
...
extrema_avg <- data_hourly %>% 
  ...
  # left_join(data_meta) %>% 
  # filter(str_detect(label, "SE \\D")) %>%
  select(date_hour, pm25_epa_2021) %>% 
  group_by(date_hour) %>% 
  summarize(pm25_epa_2021 = mean(pm25_epa_2021, na.rm = TRUE)) %>% 
  mutate(
    max = case_when(pm25_epa_2021 == max(pm25_epa_2021, na.rm=TRUE) ~ pm25_epa_2021),
    min = case_when(pm25_epa_2021 == min(pm25_epa_2021, na.rm=TRUE) ~ pm25_epa_2021),
    max = as.character(sprintf(paste0("%.", "2", "f"), round(max, digits = 2))),
...
data_hourly %>% 
  ...
  geom_line(
    data = data_hourly %>% drop_na(pm25_epa_2021),
    color = "gray",
    aes(group = site_id)
  ) +
...
data_hourly %>% 
  ...
  #   data = . %>% 
  #     group_by(label) %>% 
  #     summarize(mean = mean(pm25_epa_2021,na.rm=TRUE)),
  #   aes(yintercept = mean)
  # ) +
  # geom_hline(
  #   color = "green",
...
mid_val <- (max(data_hourly$pm25_epa_2021, na.rm=TRUE) - min(data_hourly$pm25_epa_2021, na.rm=TRUE))/2
...
# data_hourly %>% 
  ...
  distinct() %>% 
  left_join(data_meta) %>% 
  select(label, site_id, date_hour, pm25_epa_2021) %>% 
  mutate(
    extrema = case_when(
      pm25_epa_2021 == max(pm25_epa_2021, na.rm=TRUE) ~ "max",
      pm25_epa_2021 == min(pm25_epa_2021, na.rm=TRUE) ~ "min"
    )
  ) %>% 
  drop_na(extrema) %>% 
  pivot_wider(names_from = extrema, values_from = c(pm25_epa_2021, date_hour))
...
data_hourly %>% 
  ...
  ggplot(aes(x = date_hour, y = pm25_epa_2021, group = site_id)) +
  facet_grid(label~.) +
  geom_line(data = data_hourly %>% drop_na(pm25_epa_2021), color = "grey") +
  geom_point() +
  geom_point(
    data = . %>% group_by(site_id) %>% filter(pm25_epa_2021 == max(pm25_epa_2021)) %>% distinct(),
    color = "red"
  ) +
  geom_text(
    data = hourly_mm,
    ...
    aes(
      ...
      # y = (max(data_hourly$pm25_epa_2021, na.rm=TRUE) - min(data_hourly$pm25_epa_2021, na.rm=TRUE))/2,
      label = paste0(
...
single_site <- data_hourly %>%
  ...
  pivot_longer(cols = c(pm25_cf1, pm25_epa_2020, pm25_epa_2021, epa_atm, pm25_lrapa, pm25_atm),
               names_to = "corr",
               values_to = "pm25") %>% 
  mutate(
    ...
    source = fct_collapse(corr,
      ...
      EPA = c("pm25_epa_2020", "pm25_epa_2021", "epa_atm"),
      LRAPA = "pm25_lrapa"
    )
  )
...
data_diurnal %>% 
  ...
  ggplot(aes(x=hour, y=pm25_epa_2021, group=site_id)) +
  geom_path(alpha=0.5) +
...
data_daily %>% 
  ggplot(aes(x = date, y = pm25_epa_2021, group = site_id)) +
  geom_path(alpha=0.5) +
  geom_point(shape = 1, alpha = 0.3)
...
#  filter(site_id %in% tenn) %>% 
  ...
  ungroup() %>% 
  column_dt("date") %>% 
  select(label, pm25_epa_2021, date_hour, date) %>% 
  streamgraph(key="label",value="pm25_epa_2021",date="date_hour") %>% 
  sg_axis_x(1, "date_hour", "%H, %d")

data_daily %>% 
#  filter(site_id %in% tenn) %>% 
  # ungroup() %>% 
  streamgraph("site_id", "pm25_epa_2021", "date") %>% 
  sg_axis_x(1, "date", "%d")
```
...
data_hourly %>% 
  filter(site_id %in% tenn) %>% 
  ggplot(aes(x = pm25_epa_2021, y = site_id, fill = ..x..)) +
...
pal <- colorNumeric(palette = "viridis", domain = data_full$pm25_epa_2021)
stroke_pal <- colorFactor(palette = "RdYlBu", domain = data_full$DEVICE_LOCATIONTYPE)

data_labs <- data_full %>% 
  select(!c(longitude, latitude)) %>% 
  mutate_if(is.numeric, ~rounding_w_zeroes(., input_digits=2)) %>% 
  left_join(data_meta)
...
pm_popup <- paste0(data_full$label," (",data_full$latitude,", ",data_full$longitude,")",
                  ...
                  "<br><b>EPA 2020:</b> ", data_full$pm25_epa_2020,
                  "<br><b>EPA 2021:</b> ", data_full$pm25_epa_2021,
                  "<br><b>LRAPA:</b> ", data_full$pm25_lrapa,
                  "<br>(", data_full$tag, ")")
  ...
  for (date_single in date_list) {
    ...
    map <- map %>%
      addCircleMarkers(
        ...
        weight = 1,
        fillColor = ~pal(pm25_epa_2021),
        #color = ~pal(pm25_epa_2021),
        radius = 5,
        fillOpacity = 0.5,
        #stroke = FALSE,
        #radius = ~sqrt(pm25_epa_2021),
        group = date_single,
        label = ~as.character(round(pm25_epa_2021, digits=2)),
        popup = pm_popup
      )
  }
# }
...
data_hourly %>% 
  ...
  select(label, date_hour, DEVICE_LOCATIONTYPE, pm25_epa_2021) %>% 
  full_join(
    (data_deq %>%
       select(date_hour, pm25_deq) %>%
       mutate(DEVICE_LOCATIONTYPE = "deq", label = "deq") %>%
       rename(pm25_epa_2021=pm25_deq) %>% 
  ...
  ggplot(aes(x = date_hour, y = label, fill = pm25_epa_2021)) +
...
data_daily %>% 
  ...
  left_join(data_meta) %>% 
  drop_na(label) %>% 
  select(site_id, date, pm25_epa_2021, DEVICE_LOCATIONTYPE) %>% 
  drop_na(pm25_epa_2021) %>% 
  distinct() %>% 
  pivot_wider(names_from = date, values_from = pm25_epa_2021) %>% 
...
data_hourly %>% 
  ...
  ungroup() %>% 
  group_by(hour, date) %>% 
  summarize(pm = mean(pm25_epa_2021, na.rm=TRUE)) %>% 
  ggplot(aes(x = hour, y = pm, color = date, group = date)) +
  geom_line()

data_daily %>% 
  select(date, pm25_epa_2021) %>% 
  ggplot(aes(x = pm25_epa_2021)) +
  facet_wrap(~date) +
...
data_hourly %>% 
  select(site_id, date_hour, pm25_epa_2021) %>% 
  rename(pm = pm25_epa_2021) %>% 
...
data_hourly %>% 
  ...
  filter(!site_id %in% high_ids) %>% 
  left_join(data_meta) %>% 
  ggplot(aes(x = date_hour, y = pm25_epa_2021, color = DEVICE_LOCATIONTYPE, shape = DEVICE_LOCATIONTYPE, alpha = DEVICE_LOCATIONTYPE)) +
  geom_point(size = 2) +
...
data_hourly %>% 
  ...
  column_dt(c("date", "hour")) %>% 
  left_join(data_meta) %>% 
  ggplot(aes(x = hour, y = pm25_epa_2021, color = date, group = date)) +
  geom_line()
```
...
```{r,eval=FALSE}
ggplot(data_daily, aes(x = date, y = pm25_epa_2021, group = date)) +
  geom_violin() + 
...
data_daily %>% 
  ...
  ggplot(aes(x = date, y = pm25_epa_2021, group = date)) +
...
library(highcharter)
data_hourly %>% 
  filter(pm25_epa_2021 <= 200, str_detect(date_tag, "Before"), site_id %in% (data_meta %>% slice(1:10) %>% pull(site_id))) %>% 
  left_join(data_meta) %>% 
  # left_join((data_meta %>% group_by(DEVICE_LOCATIONTYPE) %>% slice(1:10))) %>%
  # drop_na(label) %>%
  hchart("streamgraph", hcaes(date_hour, pm25_epa_2021, group=label))
```

---
title: "pa_oct"
author: "Gillian McGinnis"
date: "2/13/2021"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

library(tidyverse)
library(lubridate)
library(viridis)
library(ggthemes)
```

```{r data import}
wrangled_data <- read_csv("data/wrangled_new/wrangled_data.csv")
```

```{r defining periods}
#oct_start <- as.Date("2020-10-01")
#oct_end <- as.Date("2020-10-15")
oct_start <- 1
oct_end <- 15

## Times are from start time to end-1, b/c grabbing hours from POSIXct are not rounded
# Ex. 15:08 and 15:45 are both considered hour 15, so to go to 16 one should limit to <=15

## 6am to 9am
morning_start <- 6
morning_end <- 8

## 12pm to 3pm
afternoon_start <- 12
afternoon_end <- 14

## 9pm to midnight
night_start <- 21
night_end <- 23

vars_of_interest <- names(wrangled_data)[13:22]
```


```{r subsetting and averaging data}
october_averages <- wrangled_data %>%
  rename(utc = created_at) %>%
  select(!c(X1, date, time, month, date, hour, min)) %>%
  mutate(pst = with_tz(utc, tz = "Etc/GMT+8")) %>%
  mutate(
    month = month(pst, label=TRUE),
    day = day(pst),
    hour = hour(pst),
    min = minute(pst)
    ) %>%
  filter(
    month == "Oct",
    day >= oct_start,
    day <= oct_end
    ) %>%
  mutate(
    time_group = case_when(
      hour >= morning_start & hour <= morning_end ~ "morning",
      hour >= afternoon_start & hour <= afternoon_end ~ "afternoon",
      hour >= night_start & hour <= night_end ~ "night"
      )
    ) %>%
  drop_na(time_group) %>%
  select(pst, month, day, hour, min, time_group, utc, everything()) %>%
  group_by(loc_tag, long, lat, inout, time_group) %>%
  summarize_at(vars_of_interest, mean, na.rm = TRUE) %>%
  mutate(
    time_group = factor(
      time_group,
      levels = c("morning", "afternoon", "night")
      )
    )
```

```{r saving dataset}
write.csv(october_averages, "data/wrangled_new/october_averages.csv", row.names=TRUE)
```



```{r quick viz tests}
oct_subset <- october_averages %>%
  #filter(inout == "outdoor") %>%
  filter(loc_tag != "South Tabor")

ggplot(oct_subset, aes(x = loc_tag, y = `PM2.5_ATM_ug/m3`)) +
  facet_grid(inout~time_group) +
  #facet_wrap(~time_group) +
  geom_point() +
  coord_flip()

ggplot(oct_subset, aes(x = long, y = lat, color = `PM2.5_ATM_ug/m3`, size = `PM2.5_ATM_ug/m3`)) +
  facet_grid(inout~time_group) +
  #facet_wrap(~time_group) +
  geom_point(alpha = 0.5) +
  theme_few() +
  theme(legend.position = "bottom") +
  scale_color_viridis(direction = -1)

```


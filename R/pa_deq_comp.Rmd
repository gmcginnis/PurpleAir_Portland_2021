...
se_laf$range = factor(se_laf$range, levels=c('pre','fire','post'))
```
...
lafayette_pa_deq_pm25_full <- ggplot(data=subset(se_laf_long,sensor%in%c("A","B")), aes(x=pm25,y=deq, color=sensor))+
  ...
  geom_text(data=se_laf_slopes_long, aes(x=600,y=50),label=se_laf_slopes_long$slope,check_overlap=TRUE)
  #annotate(data=se_laf_slopes_long, "text",x=600,y=50, label=slope)

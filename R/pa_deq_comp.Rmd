...
lafayette_pa_deq_pm25_extended <- ggplot(data=subset(se_laf_long,sensor=="avg"), aes(x=pm25,y=deq))+
  ...
  geom_text(data = subset(se_laf_slopes_long, sensor=="avg"), aes(x = 500, y = 50, label = slope), color = "blue", size=2)
...
lafayette_pa_deq_pm25_nofire <- ggplot(data=subset(se_laf_long,sensor=="avg" & range%in%c("pre", "post", "home")), aes(x=pm25,y=deq))+
  ...
  #theme(legend.position="bottom")+
  stat_smooth(method=lm) +
  geom_text(data = subset(se_laf_slopes_long, sensor=="avg"& range%in%c("pre", "post", "home")), aes(x = 200, y = 30, label = slope), color = "blue", size=2)
...
```{r proposal_image}
prop_subset <- se_laf_long %>%
  filter(sensor == "avg",
         cf == "atm",
         range != "home")

prop_subset_labels <- se_laf_slopes_long %>%
  filter(sensor == "avg",
         cf == "atm",
         range != "home")

lafayette_pa_deq_pm25_proposal <- ggplot(data = prop_subset, aes(x = pm25, y = deq)) +
  facet_wrap(~range, labeller = labeller(range = facet_range)) +
  geom_point(shape = 3, alpha=0.5)+
  theme_igray()+
  labs(x="PA",
       y="DEQ",
       ...
       title=lab_PM) +
  stat_smooth(method = lm) +
  geom_text(data = prop_subset_labels, aes(x = 350, y = 50, label = slope), color = "blue", size=3)

lafayette_pa_deq_pm25_proposal

ggsave("lafayette_pa_deq_pm25_proposal.png", plot=lafayette_pa_deq_pm25_proposal, path="figures/", width = 7, height = 4.5)
```

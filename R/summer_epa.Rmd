...
```{r}
data_drop_18 <- data_primary %>% 
  mutate(
    date = date(date_time),
    hour = hour(date_time)
  ) %>% 
  group_by(loc_tag, loc_ab, date, hour) %>% 
  summarize(mean = mean(pm2.5_cf1, na.rm = TRUE)) %>% 
...
remove(data_drop_18, data_subset)
```
